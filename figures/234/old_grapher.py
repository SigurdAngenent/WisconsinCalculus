...
........Most figures in the free221, free222 and free234
........lectures notes were created using grapher.py
...
from math import *
def setViewBox(xmin, ymin, xmax, ymax): #{{{1
    global ViewBox
...
def openOutputFile(name, width): # And inititialize #{{{1
...
\put(0.0, 0.0){\includegraphics{%s/%s.pdf}}
...
def arrowhead(A, B, len=10, wid=3, overshoot=1.0): #{{{1
...
def arrow(A, B, len=10, wid=3, overshoot=1.0): #{{{1
    line(A, B)
    arrowhead(A, B, len, wid, overshoot)

def movepoint(p, offset): #{{{1
    p0=p[0]+offset[0]/scale
    p1=p[1]+offset[1]/scale
    return [p0,p1]

def x_axis(arrowsize=10): #{{{1
    A=[ViewBox[0], 0.0]
    B=[ViewBox[2], 0.0]
    line(A, B)
    if (arrowsize>0):
...
def axes(arrowsize=[10,10]): #{{{1
    linewidth(0.5)
    x_axis(arrowsize[0])
    y_axis(arrowsize[1])
    linewidth(1)

def grid(XMIN, XMAX, DX, YMIN, YMAX, DY): #{{{1
    x=XMIN
    while (x < XMAX+_epsX_):
      x+=DX
      line([x, YMIN], [x, YMAX])
...
def polygonA(P): # polygonal arc #{{{1
    __polygon__(P,"stroke")

def polygonC(P): # closed polygon #{{{1
    __polygon__(P,"closepath stroke")

def polygonF(P, fillcolor=0): # filled polygon #{{{1
    if fillcolor==0:
        __polygon__(P, "fill")
    else:
        __polygon__(P,"gsave \n")
...
def setrgbcolor(rgb):#{{{1 #{{{
    global __rgbcolors__
    if isinstance(rgb, str):
         rgb = __rgbcolors__[rgb]
    outfile.write("%3f %3f %3f setrgbcolor\n"\
                  % (rgb[0]/256.0,rgb[1]/256.0,rgb[2]/256.0))
    

def setlinecap(num):#{{{1 #{{{
    outfile.write("%3d setlinecap\n" % (num)) 
    

def setlinejoin(num):#{{{1 #{{{
    outfile.write("%3d setlinejoin\n" % (num)) 
    

def setdash(dash_string):#{{{1 #{{{
...
def roundPoint(center, radius=0.02, annotation="", fillcolor='white'):#{{{1 
...
def annotate(location, offset, annotation):#{{{1 #{{{
...
def rotatebox(angle, tex): #{{{1
    return r"\rotatebox{%lf}{%s}"%(angle, tex)

def PStext(location, offset, thetext, font="Helvetica", size=9): #{{{1
    global scale,hshift,scale,vshift

...
openOutputFile("04slidingladder",180)
A=[0, 0.7]
B=[1.1, 0]
...
setrgbcolor('red')
line(B, [B[0]+0.4, 0])
arrowhead(B, [B[0]+0.4, 0],len = 0.2, overshoot=2)
line(A, [0, A[1]-0.3])
arrowhead(A, [0, A[1]-0.3],len = 0.2, overshoot=2)
linewidth(1)
...
annotate(A, [3, 3], "$A$")
annotate(B, [2, 4], "$B$")
annotate(C, [2, 2], "\\sffamily ladder with length 10 ft")

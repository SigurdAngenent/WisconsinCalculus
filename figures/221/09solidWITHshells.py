...
openOutputFile("09solidWITHshells",150)
...
linewidth(1)
drawdisc(f(0.6), 0.6, 'gray')
drawdisc(f(0.55), 0.55, 'white')
pline([0,0,f(0.65)], [0, 0, 1.25])
Q2=Q2-2*pi
linewidth(0.3)
for k in range(61):
    Q = Q1+(Q2-Q1)*k/60.0
    pline([0.6*cos(Q), 0.6*sin(Q), 0],
          [0.6*cos(Q), 0.6*sin(Q), f(0.6)])
linewidth(0.6)
pline([0.55*cos(Q1), 0.55*sin(Q1), 0],
      [0.55*cos(Q1), 0.55*sin(Q1), f(0.55)])
pline([0.55*cos(Q2), 0.55*sin(Q2), 0],
      [0.55*cos(Q2), 0.55*sin(Q2), f(0.55)])
pline([-1.3, 0, 0], [1.3, 0, 0])
linewidth(0.3)
...
polygonA( map( proj, [[0.6,0,0], [1,0,0], [1, 0, f(1)]] ) )

annotate(proj([1.2, 0, 0]), [7,-2], "$r$")
annotate(proj([0, 0, 1.2]), [2,0], "$h$")
